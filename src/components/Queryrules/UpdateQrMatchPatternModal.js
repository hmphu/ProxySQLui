...
class UpdateQrMatchPatternModal extends Component {
  constructor(props) {
    super(props);
    this.state = {
      visible: false,
  ...
  okHandler = () => {
    ...
    this.props.form.validateFields((err, values) => {
      console.log('UpdateQrMatchPatternModal->okHandler->values: ', values);
      const rule_id = parseInt(values.rule_id, 10);
      const match_pattern = values.match_pattern;
      if (!err) {
        onOk({ rule_id, match_pattern });
  ...
  render() {
    ...
    const { rule_id, match_pattern } = this.props.record;
      ...
      <span>
        ...
        >
          <Form horizontal onSubmit={this.okHandler}>
            ...
            <FormItem {...formItemLayout} label="MatchPattern">
...
export default Form.create()(UpdateQrMatchPatternModal);
